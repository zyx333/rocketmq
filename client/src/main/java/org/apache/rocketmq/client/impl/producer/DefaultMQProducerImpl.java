...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST: // 只有状态为CREATE_JUST时才会启动
                ...
                // 同一个jvm中启动多个生产者时，会把生产组和对应的生产者实例注册到同一个mQClientFactory中。
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        // producer启动时会注册一个空的topicPublishInfo，仍然会走到这个if分支
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        // 1.1 从缓存中获取master broker地址
        String brokerAddr = this.mQClientFactory.findBrokerAddressInPublish(brokerName);
        if (null == brokerAddr) {
            // 1.2 如果缓存中没有，则从namesrv更新路由信息之后，再次查询
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                // 设置重试消息的重试次数。哪些topic会以RETRY_GROUP_TOPIC_PREFIX开头？todo
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        // 1. 发送half消息
        SendResult sendResult = null;
        MessageAccessor.putProperty(msg, MessageConst.PROPERTY_TRANSACTION_PREPARED, "true"); //  标识半事务消息
